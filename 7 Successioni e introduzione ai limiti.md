...
>**Una funzione $f: \N \to \R$ si chiama successione di numeri reali.**

Più in generale si parla di successioni anche quando $f: A\subseteq \N \to \R$, con $A = \{n\in \N: n\ge n_0 \}$ per qualche $n_o \in \N$.
...
- Se $\exists M \in \R: a_n\le M , \forall n\in \N$ allora la successione è detta **limitata dall’alto**;
- Se $\exists m \in \R: a_n\ge m , \forall n\in \N$ allora la successione è detta **limitata dal basso**;
- Se $\exists m,M \in \R: m\le a_n\le M , \forall n\in \N$ allora la successione è detta **limitata**;
...
- **crescente** se $a_n \le a_{n+1} \forall n\in \N$
- **strettamente crescente** se $a_n < a_{n+1} \forall n\in \N$
- **decrescente** se $a_n \ge a_{n+1} \forall n\in \N$
- **strettamente** **decrescente** se $a_n > a_{n+1} \forall n\in \N$
...
>**Diremo che una successione $a_n$ soddisfa una proprietà $p$ definitivamente se $\exist N\in \N : a_n$ soddisfi $p, \forall n > N$.** 
...
$$\forall \epsilon >0\exist N\in \N: \forall n > N ,|a_n -\ell| < \epsilon$$ (o equivalentemente $\ell-\epsilon < a_n<\ell+\epsilon$).**
...
> **Data una successione $a_n$, se $\exist \lim_{n\to\infty}a_n=\ell\in \R$, allora tale limite è unico.**
> 
...
$$\forall M >0\exist N\in \N: \forall n > N, a_n>M$$** 
>
>**Diremo che una successione diverge a $-\infty$ e scriveremo $\lim_{n\to\infty}a_n = -\infty$ se 
$$\forall M >0\exist N\in \N: \forall n > N, a_n<-M$$** 
...
Data una successione $a_n$, se essa **converge oppure diverge**, allora diremo che $a_n$ è **regolare** e che $\exist \lim_{n\to\infty}a_n=\ell\in \overline\R$. Se invece la successione **non converge né diverge**, diremo che essa è **irregolare** (oppure **oscillante** oppure **indeterminata**) e che $\nexists \lim_{n\to\infty}a_n$.
...
> **Data una successione $a_n$, se $\exist \lim_{n\to\infty}a_n=\ell\in \overline\R$, allora tale limite è unico.**
> 
...
**Diremo che $a_n$ converge per eccesso a $\ell\in \R$ se $\lim_{n\to\infty}a_n = \ell$ e $a_n \ge \ell$ definitivamente per $n\in \N$ e scriveremo $\lim_{n\to\infty}a_n = \ell^+$ se**
**$$\forall \epsilon >0\exist N\in \N: \forall n > N ,\ell<a_n<\ell+\epsilon$$** 
>
>**Diremo che $a_n$ converge per difetto a $\ell\in \R$ se $\lim_{n\to\infty}a_n = \ell$ e $a_n \le \ell$ definitivamente per $n\in \N$ e scriveremo $\lim_{n\to\infty}a_n = \ell^-$ se**
**$$\forall \epsilon >0\exist N\in \N: \forall n > N ,\ell-\epsilon<a_n<\ell$$ **
...
$\exist \lim_{n\to\infty}a_n=\ell\in \R$.
...
2. Nel caso in cui $a_n$ non fosse limitata, sapremmo comunque che essa è monotona crescente, da cui possiamo ricavare che $a_n > a_0$, $\forall n\in \N$. Dal fatto che essa non è limitata ricaviamo che il $\sup$ della successione è $+\infty$, che per definizione significa che $\forall M>0\exist n_0 \in \N: a_{n_0} > M$. Unendo le due condizioni ricaviamo che $\forall M>0\exist n_0 \in \N: a_n (\ge a_{n_0}) > M, \forall n\in \N$, che è proprio la definizione di $\lim$.
...
>Teorema
> **Data una successione $a_n$, se $\exist \lim_{n\to\infty}a_n=\ell\in \R$, allora essa è limitata.**
> 

## 2.10 Un particolare tipo di successione: la progressione geometrica

La progressione geometrica è una successione nella  $a_n = q^n, n\in \N$.
...
> **Data una successione $a_n$, se $\exist \lim_{n\to\infty}a_n=\ell\in \overline\R$ e $\ell>0$
$\Rightarrow$  $a_n > 0$ definitivamente in $n\in \N$. 
Similmente se $\ell<0$
$\Rightarrow$ $a_n < 0$ definitivamente in $n\in \N$.**
> 
...
1. Se $\ell = +\infty$, per definizione di limite $a_n > M > 0$ definitivamente in $n\in \N$, quindi la dimostrazione si conclude qui in questo caso;
2. Se $\ell \in \R , \ell > 0$, nella definizione di limite adottiamo $\epsilon = \frac{\ell}{2}$ . Da questo ricaviamo che $a_n >  \ell - \epsilon = \frac{\ell}{2} > 0$ definitivamente in $n\in \N$.
...
> **Data una successione $a_n$, se $\exist \lim_{n\to\infty}a_n=\ell\in \overline\R$ e $a_n \ge 0$ definitivamente in $n\in \N$ $\Rightarrow$  $\ell \ge 0$. 
Similmente se $a_n \le 0$ definitivamente in $n \in \N$ $\Rightarrow$  $\ell \le 0$.**
> 

### 2.11.2 - Dimostrazione

Cerchiamo di dimostrare per assurdo che $\ell < 0$. Per quanto dimostrato al punto 2.10.1, questo deve comportare che $a_n< 0$ definitivamente in $n\in \N$, assurdo e la dimostrazione si conclude qui. 

---

<img src="https://www.notion.so/icons/circle-alternate_red.svg" alt="https://www.notion.so/icons/circle-alternate_red.svg" width="15px" /> **Osservazione**
Può essere che $\ell\ge0$ e $a_n <0$ $\forall n \in \N$, in quanto $\lim_{n\to\infty}a_n= 0$ $\ge 0$.
Parimenti se $a_n > 0$ $\forall n\in\N$ ed $\exist\lim_{n\to\infty}a_n$ $\Rightarrow$ $\ell\ge0$ e non $\ell>0$.