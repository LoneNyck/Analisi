...
Per ipotesi $a_n$ è definitivamente positiva, ossia $\exists n_0 \in \mathbb{N}: \forall n \ge n_0$  $a_n\ge0$. Di conseguenza $\forall N \ge n_0$  $S_N = S_{N-1} + a_N \ge S_{N-1}$$\mathbb{R}ightarrow$ $S_N$ è definitivamente crescente.
...
> 
> 1. **Se $\sum_{n=0}^{+\infty} a_n$  diverge $\mathbb{R}ightarrow$ $\sum_{n=0}^{+\infty} b_n$  diverge;**
> 2. **Se $\sum_{n=0}^{+\infty} b_n$  converge $\mathbb{R}ightarrow$ $\sum_{n=0}^{+\infty} a_n$  converge;**
> 
> **Inoltre se $0\le a_n \le b_n, \forall n\in N$ $\mathbb{R}ightarrow$ $\sum_{n=0}^{+\infty} a_n \le \sum_{n=0}^{+\infty} b_n$** 
> 
...
1. Se $\sum_{n=0}^{N}a_n$ diverge $\mathbb{R}ightarrow$  $s = +\infty$ ed essendo $S_N <\sigma_N, \forall N\in \mathbb{N}$, allora $s<\sigma$ 
$\mathbb{R}ightarrow \sigma = +\infty$ $\mathbb{R}ightarrow \sum_{n=0}^{N}b_n$ diverge.
2. Se $\sum_{n=0}^{N}b_n$ converge $\mathbb{R}ightarrow$  $\sigma \in \mathbb{R}$. $S_N \le \sigma_N \le \sigma$ $\mathbb{R}ightarrow S_N$ è crescente e limitata
$\mathbb{R}ightarrow \sum_{n=0}^{N}a_n$ converge
...
Per ipotesi sappiamo che il limite del rapporto delle due successione tende a 1. Nella definizione di limite adottiamo $\epsilon = \frac{1}{2}$. Allora per la definizione di limite avremo che$\exists N_1 \in \mathbb{N}: \forall n > N_1$ $1 -\epsilon < \frac{a_n}{b_n} < 1+\epsilon$ $\mathbb{R}ightarrow$ ${1\over 2} < \frac{a_n}{b_n} < {3\over 2}$. Essendo le due successioni definitivamente positive significa che $\exists N_2 \in \mathbb{N}: \forall n > N_2, a_n > 0, b_n >0$.
...
> 
> 1. **$\ell > 1\mathbb{R}ightarrow \sum_{n=0}^{+\infty} a_n$  diverge e  $\lim_{n\to\infty}{a_n}= +\infty$**
> 2. **$0\le \ell < 1\mathbb{R}ightarrow \sum_{n=0}^{+\infty} a_n$  converge e  $\lim_{n\to\infty}{a_n}= 0$**
...
1.  Assumiamo che sia $0\le \ell < 1$. Nella definizione di limite assumiamo $\epsilon = \frac{1-\ell}{2}$, da cui $\exists N \in \mathbb{N} :\forall n> N \, { a_{n+1}\over a_n} < \ell+\epsilon$. Questo significa che ${a_{n+1}\over a_n} < {1+\ell\over 2} < 1$.
    ...
    Di conseguenza $\forall n > N \ a_{n+1} < q\cdot a_n < q^2 \cdot a_{n-1}<...< q^{(n+1)-N}a_N$, che implica $a_{n+1} < {a_N\over q^N} q^{n+1}$ $\mathbb{R}ightarrow$  $0< a_{n} < {a_N\over q^N} q^{n}$.
...
2. Assumiamo ora che sia $\ell > 1$. 
    1. Se $\ell \in \mathbb{R}$, assumiamo $\epsilon = \frac{\ell-1}{2}$, da cui $\exists N \in \mathbb{N} :\forall n> N \, { a_{n+1}\over a_n} > \ell-\epsilon>1$. Questo significa che ${a_{n+1}\over a_n} > {1+\ell\over 2} > 1$.
        ...
        Di conseguenza $\forall n > N \ a_{n+1} < q\cdot a_n < q^2 \cdot a_{n-1}<...< q^{(n+1)-N}a_N$, che implica $a_{n+1} > {a_N\over q^N} q^{n+1}$ $\mathbb{R}ightarrow$   $a_{n} > {a_N\over q^N} q^{n} >1$.
