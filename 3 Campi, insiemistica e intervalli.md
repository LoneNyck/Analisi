...
> Definizione: Campo\
> **Ogni insieme $X$ dotato di due operazioni che soddisfano le proprietà di somma e prodotto viste nel paragrafo 2 del capitolo [1. Introduzione: insiemi numerici, logica e quantificatori](1%20Introduzione%20insiemi%20numerici,%20logica%20e%20quantifi%208086083780cf49709b466f3c30350a75.md) si dice campo. $\mathbb{R}, \, \mathbb{Q}, \, \mathbb{C}$ sono tutti campi.**
> 
>**Se $X$ è un insieme dotato di operazioni con tutte le proprietà di somma e prodotto e con relazione di ordine, allora si dice che $X$ è un campo ordinato. $\mathbb{Q}$ e $\mathbb{R}$ sono campi ordinati.**

Tutte le familiari regole di calcolo che utilizziamo abitualmente in $\mathbb{R}$ e in $\mathbb{Q}$ possono essere dedotte dagli assiomi di campo ordinato.
...
# 2. Numeri reali $\mathbb{R}$ e numeri razionali $\mathbb{Q}$ a confronto

Qual è il motivo per cui $\mathbb{R}$ è preferibile a $\mathbb{Q}$ per lo sviluppo dell’analisi matematica, nonostante, come visto al punto 2 di [1. Introduzione: insiemi numerici, logica e quantificatori](1%20Introduzione%20insiemi%20numerici,%20logica%20e%20quantifi%208086083780cf49709b466f3c30350a75.md) i due appaino dotati di proprietà simili e - come appena visto - entrambi siano campi ordinati? La risposta sta nel fatto che il primo soddisfa l’**assioma di continuità** mentre il secondo no - $\mathbb{R}$ è di fatto **l’unico** insieme ordinato a soddisfarlo.
...
Sia $X = \mathbb{R}\, o  \, X = \mathbb{Q}$ e consideriamo sottoinsiemi non vuoti $E \subseteq X$.
...
Se $k$ è maggiorante di $E$ e $k \in E$, $\mathbb{R}ightarrow$ $k = \max{E}$

Se $h$ è minorante di $E$ e $k \in E$, $\mathbb{R}ightarrow$ $h = \min{E}$
...
- In $E = \{ r \in \mathbb{Q} : r \ge 0, r^2 \le 2\} \sub \mathbb{R}$ ogni $h \le 0$  è minorante, ogni $k \ge \sqrt{2}$ è maggiorante.
...
**Dato $E \subseteq \mathbb{R}$ non vuoto e limitato dall’alto, $M \in \mathbb{R}$ è l’estremo superiore se:** 
...
**Dato $E \subseteq \mathbb{R}$ non vuoto e limitato dal basso, $m \in \mathbb{R}$ è l’estremo inferiore se:** 
...
$E = \mathbb{N}$ 
$\mathbb{N}exists\sup{E}$
$\min{E} = \inf{E} = 0$

$E = \{  \frac{1}{n} : n \in \mathbb{N}, n\ge1\}$ 
$\max{E} = \sup{E} = 1$
$\mathbb{N}exists \min{E}$
$\inf{E} = 0$
...
$\inf{E} = \min{E} = 0$
$\mathbb{N}exists \sup{E}$
$\mathbb{N}exists\max{E}$ 

$E = \{ r \in \mathbb{Q} : r \ge 0, r^2 \le 2\} \subseteq \mathbb{R}$
$\inf{E} = \min{E} = 0$
$\sup{E} = \sqrt{2}$
$\mathbb{N}exists\max{E} \Leftarrow \sqrt{2} \mathbb{N}otin \mathbb{Q}$  

## 4.1 Corollari derivati dalla proprietà dell’estremo superiore


> Dato $E \subseteq \mathbb{R}$ non vuoto, se esiste $\max{E}$ o $\min{E}$ allora $\max{E} = \sup{E}$ o $\min{E} = \inf{E}$

>Se  $\sup{E} \in E$, $\mathbb{R}ightarrow$ $\sup{E} = \max{E}$ e parimenti se $\inf{E} \in E \mathbb{R}ightarrow \sup{E} = \min{E}$

> Dato $E \subseteq \mathbb{R}$ non vuoto e limitato dall’alto o dal basso, $\sup{E}$ o $\inf{E}$ esiste in $\mathbb{R}$.


## 4.2 Assioma di continuità di $\mathbb{R}$


**La proprietà dell’estremo superiore di $\mathbb{R}$ nella definizione assiomatica di $\mathbb{R}$ prende il nome di assioma di continuità di $\mathbb{R}$.**

> Definizione assiomatica di $\mathbb{R}$
> **Chiamiamo $\mathbb{R}$ un insieme che soddisfa le proprietà di somma, prodotto, relazione di ordine totale e dell’estremo superiore, ossia un campo ordinato che ha la proprietà dell’estremo superiore.** \
**[Analisi I - Bramanti, Pagani, Salsa]**


### 4.2.1 Corollari derivati dall’assioma di continuità di $\mathbb{R}$

> **Corollario**
> Dato $E \subseteq \mathbb{R}$ non vuoto, se $E$  è illimitato dall’alto, diremo che $\sup{E}= +\infty$

> **Corollario**
> Dato $E \subseteq \mathbb{R}$ non vuoto, se $E$ è illimitato dal basso, diremo che $\sup{E}= -\infty$

> **Corollario**
> Dato $E \subseteq \mathbb{R}$ non vuoto, esistono sempre $\sup{E}$ e $\inf{E}$


# 5. Modulo, disuguaglianza triangolare e retta dei numeri reali

>Definizione: Modulo\
>**Si dice valore assoluto o modulo di $a \in \mathbb{R}$ il numero ≥ 0 definito da $|a| = \begin{cases} a, & a\ge0 \\ -a, &a < 0 \end{cases}$**


**$\forall x,y \in \mathbb{R}$ vale la proprietà $|x+y| \le |x| + |y|$, che prende il nome di disuguaglianza triangolare.**

<img src="https://www.notion.so/icons/circle-alternate_red.svg" alt="https://www.notion.so/icons/circle-alternate_red.svg" width="15px" /> **Osservazione**\
Spesso la disuguaglianza triangolare compare nella forma
$|a-b| \le |a-c| + |c-b|, \, \forall a,b,c, \in \mathbb{R}$, ottenuta per $x = a-c$ e $y = c-b$ 

## 5.1 La retta dei numeri reali

$\mathbb{R}$ può essere rappresentato geometricamente mettendolo in corrispondenza biunivoca con i punti di una retta orientata - e la stessa cosa potrebbe essere fatta anche per $\mathbb{Q}$, solo che la corrispondenza non sarebbe più biunivoca.
...
4. Ad ogni $a \in \mathbb{R}$ si associa il punto sulla retta che è secondo estremo di un segmento avente primo estremo O e lunghezza $|a|$. Il punto è dallo stesso lato di 1 se $a >0$ , dal lato opposto se $a <0$.
...
Geometricamente $|a|$ è la lunghezza del segmento di estremi $\overline{OA}$, ossia la distanze da $O$ ad $A$ Più in generale $|a-b|$ è la lunghezza del segmento di estremi $\overline{AB}$, cioè la distanza da $A$ a $B$, $\forall a,b \in \mathbb{R}$.
...
$(-\infty,+\infty) = \mathbb{R}$ è un **intervallo illimitato** e si dice che esso è **sia aperto che chiuso**.

# 7. Densità

> Definizone: Densità
>**$\forall x, y \in \mathbb{R}: x < y, \, \exists r \in \mathbb{Q} : x < r<y$**
**Si dice pertanto che $\mathbb{Q}$ è denso in $\mathbb{R}$.**
