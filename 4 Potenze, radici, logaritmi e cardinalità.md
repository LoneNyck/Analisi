...
## 1.1  Potenza
> Teorema
> **Sia $y \in \R: y \ge 0, n \in \N , n\ge 1$. 
Allora $\exist! x \in \R : x \ge 0$ e $x^n = y$**
>
...
$\sqrt[n]{y} \ge 0, \forall y >0, \forall n\in\N, n>0$
$\sqrt[n]{y^n} = y, \forall y > 0, \forall n\in\N, n>0$
- $\forall a>0\Rightarrow a^0 = 1$ 
$a^m = a\cdot a \cdot a...$ $m$ volte
$a^{1\over n} = \sqrt[n]{a}$
$a^{m\over n} = (a^{m})^{1\over n} = \sqrt[n]{a^m}$, $\forall a\ge0, m, n \in \N, n\neq 0$
- $\forall a \in \R, a > 0, a^{-1} = \frac{1}{a}$
$a^{-{1\over m}}  =(a^{1\over m})^{-1} = {1 \over a^{1 \over m}}$, $\forall a >0, m \in \N, m\neq 0$
</aside>
...
Data l’espansione decimale di $b$  possiamo costruire una successione $\pm a^{b_0}, \pm a^{b_0, b_1}, \pm a^{b_0, b_1b_2}, ..., \pm a^{b_0,b_1...b_k}$  con $k \in \N$ ben definita secondo le precedenti definizioni, infatti $b_0,b_1...b_k \in \mathbb{Q}$.  $a^b$ sarà quindi il limite per $k \to +\infty$ della suddetta successione. Si dimostra che $a^b$ può essere anche caratterizzato come $\sup\{ \pm a^{b_0,b_1...b_k} : k \in \N\}$ oppure $\inf\{ \pm a^{b_0,b_1...b_k} : k \in \N\}$ a seconda che la successione sia crescente o decrescente. 
...
> **Dati $y, a \in \R$ con $y > 0, a >0 , a \neq 1$ $\Rightarrow$ $\exist! x \in \R: a^x = y$.**
...
>**Dati $A, B$ insiemi, essi si dicono di uguale cardinalità se possono essere messi in corrispondenza biunivoca, ossia $\forall a \in A, \exist! b \in B.$ Nel linguaggio delle funzioni possiamo scrivere $f: A \to B$ deve essere invertibile, quindi deve essere iniettiva e suriettiva, cioè biettiva.**


## 2.1 Cardinalità numerabile
> Definizione: Cardinalità numerabile
> **La cardinalità di $\N$  e di ogni altro insieme che possa essere messo in corrispondenza biunivoca con esso è detta numerabile.**
> 
...
Una corrispondenza biunivoca tra $\N$ e $\mathbb{Z}$ può essere la seguente:

$f: \mathbb{Z} \to \N$

$\begin{matrix} \mathbb{Z} & 0 & 1 & -1 & 2 & -2 & ... & n & -n&... \\ \N& 0 & 1 & 2 & 3 & 4 & ... & 2n-1 & 2n&... \end{matrix}$
...
In una tabella infinita vengono disposte tutte le frazioni ${m \over n}, m, n \in \N, m, n \neq 0$, dove nella 
...
Costruiamo la corrispondenza biunivoca tra $\mathbb{Q}^+$ e $\N$ contando gli elementi percorrendo la tabella lungo le diagonali inverse, passando alla successiva ogni volta esauriti gli elementi della corrente, saltando inoltre quelli equivalenti a quelli già presi.
...
**Punto iv**. La corrispondenza biunivoca tra $\mathbb{Q}$ e $\N$ si costruisce in modo simile a quella vista nel 2.1.1.

$\begin{matrix} \N& 0 & 1 & 2 & 3 & 4 & ... & 2n-1 & 2n&...  \\ \mathbb{Q} & 0 & q_1 & -q_1 & q_2 & -q_2 & ... & q_n & -q_n&... \\ \end{matrix}$
...
Poiché $\R$ non è numerabile, ma $\N \sub \R$, diremo che $\R$ ha **cardinalità maggiore** di $\N$.

> Definizione: Cardinalità del continuo
>**La cardinalità di $\R$ si chiama potenza del continuo.**

<img src="https://www.notion.so/icons/circle-alternate_red.svg" alt="https://www.notion.so/icons/circle-alternate_red.svg" width="15px" /> **Osservazione**
$\R = \mathbb{Q} + \R/\mathbb{Q} \Rightarrow \R/\mathbb{Q}$  non è numerabile, ha cardinalità maggiore di $\N$ e si dimostra avere potenza del continuo.
