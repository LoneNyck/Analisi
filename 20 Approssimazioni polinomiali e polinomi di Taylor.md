...
1. $P_1(x)$. 
Se $f$ è derivabile 1 volta in $x_0\in (a,b)\mathbb{R}ightarrow$
...
2. $P_{(n-1)}(x)\mathbb{R}ightarrow P_{(n)}(x)$. 
Se $f$ è derivabile n - 1 volte in $x_0, n\ge2$, vale che $\lim_{x\to x_0}\frac{f(x) -\sum_{j= 0}^{n-1}\frac{f^{(j)}(x_0)}{j!}(x-x_0)^j}{(x-x_0)^{n-1}}=0$ come ipotesi di induzione.
    ...
    $\mathbb{R}ightarrow\lim_{x\to x_0}\frac 0 n = 0$.
...
$f'(x_0)=f''(x_0)=...=f^{(n-1)}(x_0)=0$ e $f^{(n)}(x_0)\mathbb{N}e 0$. Allora**
> 
...
- Non è detto che la serie di Taylor di $f$ centrata in $x_0$ converga, per $x\mathbb{N}e x_0$.
- Se la serie di Taylor di $f$ converge in $x\mathbb{N}e x_0$, non è detto che essa converga a $f(x)$
