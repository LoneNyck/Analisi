...
$1>0$
$\mathbb{R}ightarrow x^2 +1 \ge 1 > 0 \mathbb{R}ightarrow x^2 +1 \mathbb{N}eq 0$
...
$a$ prende il nome di **parte reale** del numero complesso e viene indicata come $\mathbb{R}e{z}$; $b$  prende invece il nome di **parte immaginaria** del numero complesso e viene indicata come $\Im z$
...
$\mathbb{R}e z , \Im z \in \mathbb{R}, \forall z \in \mathbb{C}$
...
- $\mathbb{R}e z$ costante corrisponde ad una **retta verticale** nel piano complesso;
...
- $z^{-1} = {1 \over z} \cdot { \overline z \over \overline z} ={\overline z \over |z| ^2}, \forall z \in \mathbb{C}, z \mathbb{N}eq 0$
- $\overline {w+z} = \overline z + \overline w, \forall w,z \in \mathbb{C}$
- $\overline {wz} = \overline w \cdot \overline z, \forall w,z \in \mathbb{C}$
- ${z \over w } = {z \overline w\over |w|^2}, \forall w,z \in \mathbb{C}, w \mathbb{N}eq 0$

# 5. Forma trigonometrica e forma esponenziale

Alternativamente alla forma algebrica di $z$  - in cui assegniamo $x = \mathbb{R}e z$, $y = \Im z$, da cui 
$z = x +iy \in \mathbb{C}$, possiamo individuare il numero complesso assegnando $\mathbb{R}ho = \sqrt{x^2 + y^2}$ e $\theta = \arg z$, che corrisponde all’angolo orientato formato dal semiasse delle ascisse positive e dalla semiretta uscente da $O$ e passante per $z$ - angolo positivo se in senso antiorario, negativo altrimenti.

<img src="https://www.notion.so/icons/circle-alternate_red.svg" alt="https://www.notion.so/icons/circle-alternate_red.svg" width="15px" /> **Osservazioni**
- $\arg 0$ non è ben definito.
- L’argomento di un numero complesso $z\mathbb{N}eq0$ è definito a meno di multipli interi di $2\pi$, ossia se $\arg z = \theta = \theta + 2k\pi , \forall k \in \mathbb{Z}$

Nel piano complesso $\theta$  costante corrisponde ad una **semiretta** uscente da $O$, esclusa l’origine degli assi. 

$\biggl\{\begin{aligned} &x = \mathbb{R}e z= \mathbb{R}ho \cos\theta \\  &y = \Im z= \mathbb{R}ho \sin\theta \end{aligned}$

Possiamo quindi concludere che:

$$
\begin{cases} \cos\theta = {x \over \mathbb{R}ho} = {x \over \sqrt{x^2 + y^2}}  \\  \sin\theta = {y \over \mathbb{R}ho} = {y \over \sqrt{x^2 + y^2}} \end{cases} \mathbb{R}ightarrow \tan\theta = {y \over x }
$$

<img src="https://www.notion.so/icons/circle-alternate_red.svg" alt="https://www.notion.so/icons/circle-alternate_red.svg" width="15px" /> **Osservazione**
In generale può NON essere vero che $\theta = \arccos {x \over \mathbb{R}ho}$, $\theta = \arcsin {y \over \mathbb{R}ho}$ e $\theta = \arctan {y \over x}$
...
> **Dato il numero complesso $z \in \mathbb{C}, z= x+yi$, $z = \mathbb{R}ho (\cos\theta +i\sin\theta)$ prende il nome di rappresentazione in forma trigonometrica del numero complesso.**

>Definizione: forma esponenziale
>**Dato il numero complesso $z \in \mathbb{C}, z= x+yi = \mathbb{R}ho (\cos\theta +i\sin\theta)$, $z = \mathbb{R}ho e ^{i \theta}$ prende il nome di rappresentazione in forma esponenziale del numero complesso.**
...
$z = \mathbb{R}ho (\cos\theta +i\sin\theta) = \mathbb{R}ho e^{i \theta}$ 
$\mathbb{R}ightarrow$ $z\cdot w = \mathbb{R}ho\sigma(\cos(\theta + \phi) + i \sin(\theta + \phi)) = \mathbb{R}ho \sigma e^{i (\theta + \phi)}$**
> 
> **Inoltre se $w \mathbb{N}eq 0$
> $\mathbb{R}ightarrow$  ${z\over w} = {\mathbb{R}ho \over \sigma}(\cos(\theta - \phi) + i \sin(\theta - \phi)) = {\mathbb{R}ho \over \sigma} e^{i (\theta - \phi)}$**
...
<img src="https://www.notion.so/icons/circle-alternate_red.svg" alt="https://www.notion.so/icons/circle-alternate_red.svg" width="15px" /> **Osservazione**
Se $z = 1$ $\mathbb{R}ightarrow$  ${1\over \sigma}(\cos(\phi) - i \sin(\phi)) = {1\over \sigma} e^{i (-\phi)}$

Dalla formula di De Moivre per il prodotto è possibile ricavare la formula per la potenza di un numero complesso $z$.

>Teorema 
> **Dati $z \in \mathbb{C}$, $n \in \mathbb{N}$, $z^n = \mathbb{R}ho^n (\cos n\theta +i\sin n\theta) = \mathbb{R}ho^n e ^{i n\theta}$**
> 
> **Inoltre se $z\mathbb{N}eq 0$, $z^{-n} = {1 \over z^n} (\cos n\theta -i\sin n\theta) =  {1 \over z^n}e ^{-in\theta}$**
> 
...
>**Se $w \in \mathbb{C}$, $n \in \mathbb{N}, n\ge 1$ $\mathbb{R}ightarrow$  ogni numero complesso $z \in \mathbb{C}$ tale che $z^n = w$ prende il nome di radice *n-esima* di $w$.**

> Teroema
> **Si dimostra che esistono esattamente  $n$ radici complesse *n-esime* di $w$.**

Se $w = \mathbb{R}ho e^{i\theta} = \mathbb{R}ho (\cos \theta + i \sin \theta)$, allora le radice complesse *n-esime* $z_0,z_1, z_2,...,z_n$ hanno la forma:
$z_k = \mathbb{R}ho ^ {1 \over n}(cos\phi_k + i \sin \phi_k) = \mathbb{R}ho^ {1 \over n} e^{i\phi_k}$ con $\phi_k = {1 \over n}(\theta + 2k\pi)  = { \theta + 2k\pi \over n}$, per 
$k = 0,1,...,n-1$
...
Le radici *n-esime* di un numero complesso $w \in \mathbb{C}, w \mathbb{N}eq 0$ formano un **insieme** di $n$  numeri complessi.
...
Date $a,b,c, \in \mathbb{C}, a\mathbb{N}eq0$ $\mathbb{R}ightarrow$  $az^2+bz+c = 0$ ha necessariamente due soluzioni per quanto detto nel paragrafo precedente.
...
$a_n, a_{n-1},...,a_1, a_0 \in \mathbb{C}, a_n \mathbb{N}eq 0, n\in \mathbb{N}, n\ge1$  ha esattamente $n$ soluzioni in $\mathbb{C}$ se ciascuna di esse è contata con le dovute molteplicità - ossia le radici possono essere non tutte distinte.**
> 
