...
>**Date due funzioni $f$  e $g$ e $x_0 \in \overline \R$ diremo che**
...
- $\lim_{x\to x_0}{f(x)\over g(x)} = \lambda \in \R \setminus \{0\}$ $\Rightarrow$ $f(x) \sim \lambda g(x)$ , $x\to x_0$.
...
2. È possibile riscrivere $f(x) = \frac{f(x)}{g(x)}g(x)$. Esistendo il limite di $g(x)$ per $x\to x_0$ in $\overline \R$, per algebra dei limiti ed aritmetizzazione di infinito possiamo scrivere che $\lim_{x\to x_0}\frac{f(x)}{g(x)}g(x)= 1\cdot\ell = \ell$ essendo le due funzioni asintotiche per ipotesi.
